...
import OrderSkeleton from '@/components/layout/OrderSkeleton.vue'
...
<template>
    <section class="mx-auto w-[90%] mt-16 mb-24">
        ...
        <div v-else>
            <div v-if="displayedOrders.length === 0" class="text-center py-40 rounded-xl">
                <h2 class="text-2xl font-black uppercase tracking-[0.3em] mb-12">No Orders Found</h2>
                <RouterLink to="/products"
                    class="bg-black text-white px-10 py-4 text-[10px] font-black tracking-[0.2em] uppercase">Start
                    Shopping
                </RouterLink>
            </div>
                ...
                <div class="bg-[whitesmoke] px-8 py-6 flex justify-between items-center">
                    <div class="flex gap-12">
                        ...
                        <div>
                            ...
                            <p class="font-mono text-[10px] text-gray-600 mt-1">#{{ order.$id.slice(-8).toUpperCase() }}
                            </p>
                        </div>
                    </div>
                ...
                <div class="p-8 flex flex-col gap-6">
                    <div v-for="item in order.items" :key="item.id" class="flex gap-6 items-center">
                        ...
                        <div class="flex flex-col gap-1">
                            <p class="font-bold uppercase text-sm text-[#3A3A3A]">{{ item.name }} (x{{ item.quantity }})
                            </p>
